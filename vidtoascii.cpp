...
struct s_Frame {
    int nXSize;
    int nYSize;                            // Basically the resolution and the frame itself.
...
// Declaration of Functions. Not Independence.
void useWebcam();
...
// Variables
float dx = 0.2; // These variables decrease resolution to something acceptable.
float dy = 0.2; 
...
int main(int argc, char *argv[]) {
    ...
    if (argc > 3) { // 3 if-statements are not enough.
...
void useFile(str sFile_name) {
    ...
    while (true) {
        ...
        cv::resize(frame, frame, cv::Size(), dx, dy, cv::INTER_LANCZOS4);
...
void useWebcam() {
    ...
    while (true) {
        ...
        cv::resize(web_frame, web_frame, cv::Size(), dx, dy, cv::INTER_LANCZOS4);
